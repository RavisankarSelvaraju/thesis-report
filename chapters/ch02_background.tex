...
\begin{document}
    ...
    \begin{flushleft}
        ...
        $ \xi_{A}^{F} - $ Twist of moving Frame or point A represented in Frame F \\
    ...
    For a transformation matrix $T_{body}^{world}$ and a twist $\boldsymbol{\xi}_{body}^{body}$ represented in the body frame,  
    ...
    The Adjoint transformation for the transformation matrix \ref{TransformationOfBodyToWorld} is given by the equation \ref{AdjointTransformationEquation}
    ...
    If we need to transform a twist of the body frame represented in the body frame $\boldsymbol{\xi}_{body}^{body}$ to the twist of body frame represented in the world frame $\boldsymbol{\xi}_{body}^{world}$, we can use the following equation \ref{eq:TwistTransformationEquation}. 
    If we want to change the frame of representation then, the rotation between frame need to be included in the transformation matrix, but if only the reference point of twist need to be changed only the position vector need to be changed.
    
    \begin{equation}
        \boldsymbol{\xi}_{body}^{world} = \text{Ad}_{T_{body}^{world}} \boldsymbol{\xi}_{body}^{body}
